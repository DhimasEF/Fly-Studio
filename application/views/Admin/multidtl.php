<div class="content">   
	<div class="bg justify-between align-center">
		<h1 class="f3">
			<a class="no-deco" href="<?= base_url('Admin/collaboration') ?>"><i class='bx bxs-videos'></i>Detail Collaboration</a> /
		...
		<div class="contentdtl-title">
			...
			<div class="ctndtl-act">
				<?php if ($user->role == "admin"): ?>
					<p>
						<a href="<?= base_url('Admin/collaboration/edit/' .rawurlencode(base64_encode( $content->id_file))); ?>">
							<i class="bx bx-edit"></i>
						</a>
						<a href="<?= base_url('Admin/collaboration/manage_participant/' . rawurlencode(base64_encode($content->id_file))); ?>">
	...
	<div class="ctndtl-container">
			...
			<?php foreach ($comments as $comment): ?>
				...
				<div class="comment">
						...
						<div class="cmt-ctn-txt">
							...
							<div class="del-cmt">
								...
								<?php if ($comment['id_user'] == $this->session->userdata('user_id')): ?>
									<form action="<?= base_url('Admin/collaboration/delete_comment/' . $comment['id_comment']) ?>" method="POST" style="display:inline;" onsubmit="return confirm('Are you sure you want to delete this comment?');">
					...
					<div class="replies-cmt">
						<!-- Form reply -->
						<form action="<?= base_url('Admin/collaboration/add_comment/' . rawurlencode(base64_encode($content->id_file)) . '/' . $comment['id_comment']) ?>" method="POST">
										...
										<div class="cmt-ctn-txt">
											...
											<div class="del-cmt">
												...
												<?php if ($reply['id_user'] == $this->session->userdata('user_id')): ?>
													<form action="<?= base_url('Admin/collaboration/delete_comment/' . $reply['id_comment']) ?>" method="POST" style="display:inline;" onsubmit="return confirm('Are you sure you want to delete this reply?');">
														<button type="submit" style="color:red; background:none; border:none; cursor:pointer;"><i class="bx bx-trash"></i></button>
													</form>
												<?php endif; ?>
											</div>
		...
		<div class="comment-input">
			...
			<?php else: ?>
				<img src="<?= base_url('.assets/uploads/Profil/default.jpg'); ?>" alt="Foto Default" class="ctndtl-avt">
			<?php endif; ?>
			<form action="<?= base_url('Admin/collaboration/add_comment/' . rawurlencode(base64_encode($content->id_file))); ?>" method="POST">
...
<script>
    ...
    $(document).on('click', '#like-button', function() {
        ...
        $.ajax({
            url: "<?= base_url('Admin/collaboration/toggle_like/') ?>" + id_content,
            method: "POST",
