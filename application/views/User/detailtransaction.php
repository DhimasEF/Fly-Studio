<div class="content">   
	<div class="bg justify-between align-center">
		<h1 class="f3">
			<a class="no-deco" href="<?= base_url('User/transaction') ?>"><i class='bx bx-transfer'></i>List Transaction</a> /
	...
	<div class="trx-container">
		<div class="trx-info">
			<div class="trx-data">
				...
				<?php if ($is_worker): ?>
					<a class="trx-button" href="<?= site_url('User/transaction/edit/' . rawurlencode(base64_encode($transaction->id_transaction))); ?>">
		...
		<?php if ($is_orderer && !$is_paid): ?>
			...
			<div class="mt-2">
				<form class="trans-edit" action="<?= base_url('User/transaction/pay/' . $transaction->id_transaction) ?>" method="post" enctype="multipart/form-data">
		...
		<div class="payment-data">
			...
			<div class="note-btn">
				<?php if (($is_orderer && $is_paid && $transaction->order_status === 'completed') || $is_worker): ?>
					<a class="trx-button" href="<?= base_url('User/transaction/print_invoice_pdf/' . rawurlencode(base64_encode($transaction->id_transaction))) ?>" target="_blank">
						<i class='bx bx-search-alt'></i> Preview Nota
					</a>
					<a class="trx-button" href="<?= base_url('User/transaction/download_invoice_pdf/' . rawurlencode(base64_encode($transaction->id_transaction))) ?>" target="_blank">
						...
						<?php if ($is_worker && $has_pending): ?>
							<td class="row-btn">
								<?php if ($payment->payment_status === 'pending') : ?>
									...
									<a href="<?= base_url('User/transaction/accept/' . $encoded_id) ?>">
										<i class='bx bx-check-circle' style="color: green;"></i>
									</a>
									<a href="<?= base_url('User/transaction/reject/' . $encoded_id) ?>">
