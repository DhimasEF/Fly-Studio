<div class="content">   
	<div class="bg justify-between align-center">
		<h1 class="f3">
			<a class="no-deco" href="<?= base_url('Creator/content') ?>"><i class='bx bxs-videos'></i>Detail Content</a> /
        ...
        <div class="contentdtl-title">
			...
			<div class="ctndtl-act">
				<?php if ($content->id_uploader == $this->session->userdata('user_id')): ?>
					<p>
						<a href="<?= site_url('Creator/content/edit/' . rawurlencode(base64_encode($content->id_content))) ?>"><i class="bx bx-edit"></i></a>
						<a href="<?= site_url('Creator/content/delete/' . rawurlencode(base64_encode($content->id_content))) ?>" onclick="return confirm('Are you sure you want to delete this content?');"><i class="bx bx-trash"></i></a>
					</p>
				<?php endif; ?>
			</div>
		</div>
    ...
    <div class="ctndtl-container">
			...
			<?php foreach ($comments as $comment): ?>
				...
				<div class="comment">
						...
						<div class="cmt-ctn-txt">
							...
							<div class="del-cmt">
								...
								<?php if ($comment['id_user'] == $this->session->userdata('user_id')): ?>
									<form action="<?= base_url('Creator/content/delete_comment/' . $comment['id_comment']) ?>" method="POST" style="display:inline;" onsubmit="return confirm('Are you sure you want to delete this comment?');">
					...
					<div class="replies-cmt">
						<!-- Form reply -->
						<form action="<?= base_url('Creator/content/add_comment/' . rawurlencode(base64_encode($content->id_content)) . '/' . $comment['id_comment']) ?>" method="POST">
										...
										<div class="cmt-ctn-txt">
											...
											<div class="del-cmt">
												...
												<?php if ($reply['id_user'] == $this->session->userdata('user_id')): ?>
													<form action="<?= base_url('Creator/content/delete_comment/' . $reply['id_comment']) ?>" method="POST" style="display:inline;" onsubmit="return confirm('Are you sure you want to delete this reply?');">
														<button type="submit" style="color:red; background:none; border:none; cursor:pointer;"><i class="bx bx-trash"></i></button>
													</form>
												<?php endif; ?>
											</div>
		...
		<div class="comment-input">
			...
			<?php else: ?>
				<img src="<?= base_url('.assets/uploads/Profil/default.jpg'); ?>" alt="Foto Default" class="ctndtl-avt">
			<?php endif; ?>
			<form action="<?= base_url('Creator/content/add_comment/' . rawurlencode(base64_encode($content->id_content))); ?>" method="POST">
...
<script>
    ...
    $(document).on('click', '#like-button', function() {
        ...
        $.ajax({
            url: "<?= base_url('Creator/content/toggle_like/') ?>" + id_content,
            method: "POST",
