    <div id="chat-container">
        ...
        <div id="member-list">
			<div class="mmb-list-top">
				...
				<?php if ($is_admin): ?>
					<a href="<?= site_url('Creator/chat/addMember/' . $room_id); ?>" class="no-deco">
                            ...
                            <?php if ($is_admin): ?>
                                <div class="member-actions">
                                    <?php if ($member['status'] == 'member'): ?>
                                        <a href="<?= site_url('Creator/chat/promote_to_admin/' . $room_id . '/' . $member['id_user']) ?>" title="Make Admin">
                                            <i class='bx bxs-up-arrow-alt'></i>
                                        </a>
                                    <?php endif; ?>
                                    <?php if ($member['id_user'] != $current_user_id): ?>
                                        <a href="<?= site_url('Creator/chat/remove_member/' . $room_id . '/' . $member['id_user']) ?>" title="Remove Member">
        ...
        <div id="message-section">
            ...
            <!-- Send Form -->
            <form id="send-message-form" method="POST" action="<?= site_url('Creator/chat/send_message_group/' . $room_id); ?>">
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (form) {
        form.addEventListener('submit', function (e) {
            ...
            fetch("<?= site_url('Creator/chat/ajax_update_group_icon') ?>", {
                method: "POST",
                body: formData
            })
            .then(res => res.json())
...
function loadMessages() {
    $.ajax({
        url: "<?= site_url('Creator/chat/fetch_messages_group/' . $room_id) ?>",
        method: "GET",
