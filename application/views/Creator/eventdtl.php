			...
			<?php if ($is_special_event): ?>
				<div class="justify-between mt-2">
					...
					<?php if (!$is_event_ended): ?>
						<?php if ($has_joined): ?>
							<a href="<?= site_url('Creator/event/leave/' . rawurlencode(base64_encode($event->id_event))); ?>" onclick="return confirm('Are you sure you want to leave this event?')">
								<button class="btn-lj gap-1">
									<i class='bx bx-log-out'></i> Leave
								</button>
							</a>
						<?php elseif ($current_participants < $event->max_participants): ?>
							<a href="<?= site_url('Creator/event/join/' . rawurlencode(base64_encode($event->id_event))); ?>" onclick="return confirm('Are you sure you want to join this event?')">
								<button class="btn-lj gap-1">
            ...
            <?php if ($is_recruitment && $is_event_active): ?>
				<div class="justify-between mt-2">
					...
					<span>Go to List Recruitment</span>
						<a href="<?= site_url('Creator/listrecruitment/list'); ?>">
							<button class="btn-lj gap-1">
