		...
		<div class="event-dtl-container1 mt-2">
			...
			<div class="banner-eventdtl" style="background-image: url('<?= $banner_url ?>');">
					...
					<div class="justify-between mt-2">
						...
						<?php if (!$is_event_ended): ?>
							<?php if (!$this->session->userdata('id_user')): ?>
								<span class="text-gray f5"><em>Please log in to join or leave the event.</em></span>
							<?php elseif ($has_joined): ?>
								<a href="<?= site_url('admin/event/leave/' . rawurlencode(base64_encode($event->id_event))); ?>" onclick="return confirm('Are you sure you want to leave this event?')">
									<button class="btn-lj gap-1">
										<i class='bx bx-log-out'></i> Leave
									</button>
								</a>
							<?php elseif ($current_participants < $event->max_participants): ?>
								<a href="<?= site_url('admin/event/join/' . rawurlencode(base64_encode($event->id_event))); ?>" onclick="return confirm('Are you sure you want to join this event?')">
									<button class="btn-lj gap-1">
				...
				<?php if ($is_recruitment && $is_event_active): ?>
					<div class="justify-between mt-2">
						<?php if (!$this->session->userdata('id_user')): ?>
							<span class="text-gray f5"><em>Please log in to apply for this recruitment.</em></span>
