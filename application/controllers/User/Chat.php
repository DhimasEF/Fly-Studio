...
class Chat extends Middle_Controller {
    ...
    public function index() {
        ...
        $this->load->view('User/listchat', $data);
	...
	public function groom($room_id) {
		...
		$this->load->view('User/sidebar', $data);
		$this->load->view('User/grouproom', $data);
		$this->load->view('User/footer', $data);
	}
	...
	public function send_message_private($room_id = null) {
		...
		redirect('User/chat/proom/' . $room_id);
	}
	...
	public function send_message_group($room_id = null) {
		...
		// Redirect kembali ke halaman chat room
		redirect('User/chat/groom/' . $room_id);
	}
	...
	public function send_request($receiver_id = null) {
		...
		if ($existing_room) {
			...
			redirect('User/chat/proom/' . $existing_room->id_room); // ganti sesuai role jika perlu
			return;
		}
		...
		if ($existing_request) {
			...
			redirect('User/creator/detail/' . rawurlencode(base64_encode($receiver_id)));
			return;
		}
	...
	public function addMember($room_id) {
		...
		$this->load->view('User/sidebar', $data);
		$this->load->view('User/addmembergroup', $data);
		$this->load->view('User/footer', $data);
	}
	...
	public function storeMembers($room_id) {
		...
		if (empty($selected_users)) {
			redirect('User/chat/add_member/' . $room_id);
		}
