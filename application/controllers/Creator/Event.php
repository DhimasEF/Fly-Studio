...
class Event extends Middle_Controller {
    ...
    public function index() {
		...
		$data['scope'] = $scope;
		$data['events'] = $this->M_Event->getAllEvents($scope, 'creator');
    ...
    public function detail($encoded_id) {
        ...
        $this->load->view('Creator/sidebar', $data);
        $this->load->view('Creator/eventdtl', $data);
        $this->load->view('Creator/footer', $data);
    }
    ...
    public function join($encoded_id) {
        ...
        redirect('Creator/event/detail/' . rawurlencode(base64_encode($id_event)));
    }
