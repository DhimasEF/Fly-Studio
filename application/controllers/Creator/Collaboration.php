...
class Collaboration extends Middle_Controller {
    ...
    public function index() {
        ...
        $this->load->view('Creator/listmultiproject', $data);
	...
	public function view($encoded_id_content) {
		...
		// Load the view
		$this->load->view('Creator/sidebar', $data);
		$this->load->view('Creator/multidtl', $data);
		$this->load->view('Creator/footer', $data);
	}
	...
	public function add_comment($encoded_id_content, $id_parent = null) {
		...
		if ($this->form_validation->run() === FALSE) {
			...
			redirect('Creator/collaboration/view/' .  rawurlencode(base64_encode($id_file)));
			return;
		}
		...
		if ($id_parent) {
			...
			if (!$parent_comment) {
				...
				redirect('Creator/collaboration/view
