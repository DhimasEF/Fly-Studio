...
class Transaction extends Middle_Controller {
	...
	public function index() {
        ...
        $this->load->view('Creator/listtransaction', $data);
	...
	public function add() {
		...
		$this->load->view('Creator/sidebar', $data);
		$this->load->view('Creator/addtransaction', $data);
		$this->load->view('Creator/footer');
	}
	...
	public function pay($transaction_id) {
		...
		if ($_SERVER['REQUEST_METHOD'] === 'POST') {
			...
			if (!$this->upload->do_upload('payment_proof')) {
				...
				redirect('Creator/transaction/detail/' . rawurlencode(base64_encode($transaction_id)));
			}
	...
	public function detail($encoded_id) {
		...
		// Load view
		$this->load->view('Creator/sidebar', $data);
		$this->load->view('Creator/detailtransaction', $data);
		$this->load->view('Creator/footer', $data);
	}
    ...
    public function edit($encoded_id) {
		...
		$data['transaction'] = $this->M_Transaction->get_transaction($decoded_id);

		if (!$data['transaction']) {
			show_404();
		}

		// Pastikan hanya worker dari transaksi ini yang bisa edit
		if ($data['transaction']->id_worker != $user_id) {
			show_error("Anda tidak memiliki izin untuk mengedit transaksi ini.", 403);
		}

		// Proses form jika disubmit
		if ($this->input->post()) {
			$update_data = [
				'order_file_url' => $this->input->post('file_url'),
				'password' => $this->input->post('password'),
				'total_price' => $this->input->post('total_price'),
				'order_status' => $this->input->post('order_status'),
			];

			if ($this->M_Transaction->update_transaction($decoded_id, $update_data)) {
			...
			redirect('Creator/transaction/detail/' . rawurlencode(base64_encode($decoded_id)));
		}
		...
		$data['title'] = 'Edit Transaction ' . ($data['transaction']->id_transaction . ' | Fly Studio');

        $this->load->view('Creator/sidebar', $data);
        $this->load->view('Creator/edittransaction', $data);
        $this->load->view('Creator/footer', $data);
    }

    public function accept($encoded_id) {
        $decoded_id = base64_decode(rawurldecode($encoded_id));

        if (!$decoded_id || !preg_match('/^[a-zA-Z0-9]+$/', $decoded_id)) {
            die("Invalid Payment ID: " . htmlspecialchars($decoded_id));
        }

        $payment = $this->M_Payment->get_payment_by_id($decoded_id);
        if (!$payment) {
            die("Payment not found in database! ID: " . htmlspecialchars($decoded_id));
        }

        $this->M_Payment->update_payment_status($decoded_id, 'approved');
        $this->M_Transaction->update_total_paid($payment->id_transaction, $payment->amount);

        $this->session->set_flashdata('success', 'Pembayaran berhasil diterima.');
        redirect($_SERVER['HTTP_REFERER']);
    }

    public function reject($encoded_id) {
		$decoded_id = base64_decode(rawurldecode($encoded_id));
		...
		if (!$payment || $payment->payment_status !== 'pending') {
			$this->session->set_flashdata('error', 'Pembayaran tidak valid atau sudah diproses.');
			redirect($_SERVER['HTTP_REFERER']);
		}

		$this->M_Payment->update_payment_status($decoded_id, 'rejected');
	...
	public function print_invoice_pdf($encoded_id) {
		...
		$html = $this->load->view('Creator/invoice_pdf', $data, true);
