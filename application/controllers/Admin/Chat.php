...
class Chat extends Middle_Controller {
	...
	public function index() {
		...
		$this->load->view('Admin/listchatroom', $data);
    ...
    public function send_request($receiver_id = null) {
		...
		if ($existing_room) {
			...
			redirect('Admin/chat/proom/' . $existing_room->id_room); // ganti sesuai role jika perlu
			return;
		}
		...
		if ($existing_request) {
			...
			redirect('Admin/creator/detail/' . rawurlencode(base64_encode($receiver_id)));
			return;
		}
	...
	public function proom($room_id) {
		...
		// Load views
		$this->load->view('Admin/sidebar', $data);
		$this->load->view('Admin/chatroom', $data); // ini harus gunakan layout 2 kolom
		$this->load->view('Admin/footer', $data);
	}
	...
	public function groom($room_id) {
		...
		$this->load->view('Admin/sidebar', $data);
		$this->load->view('Admin/grouproom', $data);
		$this->load->view('Admin/footer', $data);
	}
	...
	public function createGroupRoom() {
		...
		$data['user'] = $this->M_User->get_admin_by_id($user_id); // Ambil data terbaru dari database
        // Ambil daftar user untuk ditampilkan di form
        $data['users'] = $this->M_User->get_all_users_except_me($user_id);

        // Load view untuk form pembuatan grup
        $this->load->view('Admin/sidebar', $data);
        $this->load->view('Admin/creategroup', $data);
        $this->load->view('Admin/footer', $data);
    }
	
	public function storeGroup() {
		$group_name = $this->input->post('group_name');
		$user_ids = $this->input->post('user_id'); // Array ID user anggota grup
		$creator_id = $this->session->userdata('user_id'); // ID user yang membuat grup

		if (empty($group_name) || empty($user_ids) || empty($creator_id)) {
			show_error('Group name, members, and creator are required.', 400);
		}

		// 1. Buat grup baru
		$group_data = [
			'id_room' => $this->M_Chat->id_crr(),
			'name' => $group_name,
			'type' => 'group'
		];
		$group_id = $this->M_Chat->create_group_room($group_data);
		if (!$group_id) {
			show_error('Failed to create group.', 500);
		}

		// 2. Daftarkan anggota ke grup dengan peran
		$members_data = [];
		
		// Tambahkan pembuat grup sebagai admin
		$members_data[] = [
			'id_chat_room' => $group_id,
			'id_user' => $creator_id,
			'status' => 'admin' // Menandai sebagai admin
		];

		// Tambahkan anggota lain sebagai member
		foreach ($user_ids as $user_id) {
			if ($user_id != $creator_id) { // Hindari duplikasi
				$members_data[] = [
					'id_chat_room' => $group_id,
					'id_user' => $user_id,
					'status' => 'member' // Menandai sebagai member
				];
			}
		}
	...
	public function addMember($room_id) {
		...
		$this->load->view('Admin/sidebar', $data);
		$this->load->view('Admin/addmembergroup', $data);
		$this->load->view('Admin/footer', $data);
	}
	...
	public function storeMembers($room_id) {
		...
		if (empty($selected_users)) {
			redirect('Admin/chat/add_member/' . $room_id);
		}
		...
		$this->M_Chat->add_members_to_group($members_data);
		redirect('Admin/chat/groom/' . $room_id);
	}
	...
	public function send_message_private($room_id = null) {
		...
		redirect('Admin/chat/proom/' . $room_id);
	}
